...
const nextConfig = {
  ...
  //    此时: 适合 Node/Edge (SSR + ISR), 需要 node-runtime / edge runtime 作为 nextjs 的 runtime
  // - 'standalone':
  ...
  //    此时: 适合 Docker (SSR + ISR), 为降低 image size, 会尽可能剔除不需要的 node module
  // - 'export':
  ...
  //    此时: 适合 Nginx (SSG), 需要托管在 github pages / nginx / amazon S3 bucket ... 这类静态环境中
